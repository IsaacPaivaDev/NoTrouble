...
from .models import (
    Board, Stage, Card, CardLog, ChecklistItem, Tag, User,
    Comment, Attachment, Company, VerificationCode, ActivityLog, TeamInvite,
)
...
class CardOutSchema(Schema):
    """Schema reduzido usado em listagem (Board kanban)."""
    id: uuid.UUID
    title: str
    stage_id: uuid.UUID
    tags: List[TagSchema] = []
...
def register_user(request, payload: RegisterSchema):
    if User.objects.filter(username=payload.email).exists():
        return api.create_response(request, {"success": False, "message": "Email ja cadastrado"}, status=400)
    ...
    Stage.objects.create(name="Concluido", board=board, order=2)
...
def verify_code(request, payload: VerifySchema):
    ...
    try:
        ...
        return api.create_response(request, {"success": False, "message": "Codigo invalido!"}, status=400)
    except Exception:
        return api.create_response(request, {"success": False, "message": "Nenhum codigo gerado para este usuario."}, status=400)
...
def update_card(request, card_id: uuid.UUID, payload: CardUpdateSchema):
    ...
    card.priority = payload.priority or 'medium'
...
def patch_card(request, card_id: uuid.UUID, payload: CardPatchSchema):
    card = get_card_for_user(request.auth, card_id)
...
def create_stage(request, payload: StageCreateSchema):
    board = get_board_for_user(request.auth, payload.board_id)
